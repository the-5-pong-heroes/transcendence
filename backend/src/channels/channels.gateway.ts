...
export class ChannelsGateway {
  ...
  async wantJoin(client: Socket, payload: UserWantJoinChannelDto) {
    ...
    if (channel.type === "PROTECTED") await this.channeluserService.create({ ...channelUser, isAuthorized: false });
    else await this.channeluserService.create(channelUser);
  ...
  async handleMessage(client: Socket, payload: CreateMessageDto) {
    ...
    if (payload.senderId) {
      ...
      const channelUser = await this.channeluserService.findFirst({ userId: sender.id, channelId: channel.id });
